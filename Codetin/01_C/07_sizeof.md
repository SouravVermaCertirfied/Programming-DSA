...
- There will be no symbols for this in elf.
...
## WAP : Find the size of a data type without using size of operator
This can be done by:  
...
int main(){
    int arr[2];
    int size = ((char *)&arr[1]) - ((char *)&arr[0]); // one liner
    printf("%d \n", size);
    
    char *ptr1 = (char *)&arr[0] ; // expanded explained
...
## WAP : Find the size of a a variable without using size of operator
