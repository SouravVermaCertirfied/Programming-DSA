...
- In C: all these tree styles are valid.
    ...
    - Taking the address `&foo` of a function is implicit.
```c
int foo(int x) {
    return x + 1;
}

int (*fptr1)(int) = foo; // recommended
int (*fptr2)(int) = *foo;
int (*fptr3)(int) = &foo;
...
// STEP 1 : Create a typedef for readability
...
// STEP 3 : Put functions in table (with typedef)
funDataType fun_tbl[] = {
    foo,
    boo
};

// STEP 3 (alternative) : without using typedef
void (*fun_tbl2[])(void) = {
    foo,
    boo
};
